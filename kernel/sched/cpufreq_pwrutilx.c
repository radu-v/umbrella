...
#else
static inline bool pwrgov_cpu_is_busy(struct pwrgov_cpu *sg_cpu)
{
	return false;
}
...
static void pwrgov_update_single(struct update_util_data *hook, u64 time,
				 unsigned int flags)
{
	struct pwrgov_cpu *sg_cpu =
		container_of(hook, struct pwrgov_cpu, update_util);
	struct pwrgov_policy *sg_policy = sg_cpu->sg_policy;
	struct cpufreq_policy *policy = sg_policy->policy;
...
{
	struct pwrgov_policy *sg_policy =
		container_of(work, struct pwrgov_policy, work);
...
static inline struct pwrgov_tunables *
to_pwrgov_tunables(struct gov_attr_set *attr_set)
{
...
static struct attribute *pwrgov_attributes[] = { &up_rate_limit_us.attr,
...
static struct pwrgov_tunables *
pwrgov_tunables_alloc(struct pwrgov_policy *sg_policy)
{
	struct pwrgov_tunables *tunables;
